// JavaScript source code with fixed stage2 state lookup
...
for (let block = 0; block < num_trials / trials_per_block; block++) {
  ...
  for (let j = 0; j < trials_per_block; j++) {
    ...
    // Stage 2 - fixed lookup from stage1 data
    const stage2 = {
      type: jsPsychHtmlKeyboardResponse,
      stimulus: function() {
        // Always fetch most recent stage1 for this trial
        const last_stage1 = jsPsych.data.get().filter({ stage: 1, trial: i+1 }).values()[0];
        const state = last_stage1?.state2 ?? 0;
        const symbols = [['🔵','🟡'], ['🟢','🟣']];
      ...
      on_finish: function(data) {
        // same lookup for reward calculation
        const last_stage1 = jsPsych.data.get().filter({ stage: 1, trial: i+1 }).values()[0];
        const state = last_stage1?.state2 ?? 0;
    ...
    // Feedback - press space to continue
    const feedback = {
      type: jsPsychHtmlKeyboardResponse,
      stimulus: function() {
    ...
    if (j === insert_index) {
      ...
      const memory_trial = {
        ...
        on_finish: function(data) {
          const actual = jsPsych.data.get().filter({ stage: 1, trial: i+1 }).values()[0]?.choice_stage1;
      ...
      const gamble = {
        ...
        on_finish: function(data) {
          const mem = jsPsych.data.get().filter({ stage: 'memory', trial: i+1 }).values()[0];
