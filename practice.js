...
for (let block = 0; block < num_trials / trials_per_block; block++) {
  ...
  for (let j = 0; j < trials_per_block; j++) {
    ...
    const stage1 = {
      type: jsPsychHtmlKeyboardResponse,
      stimulus: `<div style="font-size:${TEXT_SIZE}"><p>【ステージ1】ここで🔺または🔶のどちらかを選択します。</p>` +
    ...
    const stage2 = {
      ...
      stimulus: function() {
        ...
        const symbols = [['🔵','🟡'], ['🟢','🟣']];
        return `<div style="font-size:${TEXT_SIZE}"><p>【ステージ2】前の選択に応じてこちらの2つから選択します。</p>` +
    ...
    if (j === insert_index) {
      block_timeline.push(
        stage1, stage2, feedback,
        pre_memory, memory_trial, gamble, post_memory
      );
    } else {
